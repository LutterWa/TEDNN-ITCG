            ...
            with tf.name_scope('actor'):
                def _build_anet(state, name, trainable):
                    with tf.variable_scope(name):
                        l1 = tf.layers.dense(state, 64, tf.nn.relu, use_bias=False, trainable=trainable)
                        l1 = tf.layers.dense(l1, 64, tf.nn.relu, use_bias=False, trainable=trainable)
                        mean = action_bound * tf.layers.dense(l1, action_dim, tf.nn.tanh, use_bias=False,
                                                              # kernel_initializer=tf.initializers.constant(value=0.1),
                                                              trainable=trainable)
