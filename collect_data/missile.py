...
class MISSILE:
    def __init__(self, missile=None, target=None, k=1.0):
        if missile is None:
            missile = [0., 300., 45. / RAD, -20000., 10000, 200]  # 0.时间,1.速度,2.弹道倾角,3.导弹x,4.导弹y,5.质量
        if target is None:
    ...
    def step(self, action=0):
        ...
        if self.Y[0] < 400:
            ...
            # 制导指令
            m_max = 3 * g
            self.ac = ac = np.clip(3 * v * qdot + math.cos(theta) * g + action, -m_max,
                                   m_max)  # + 2 * v * (q - self.qt) / tgo
